...
const WebsitePreview = () => {
    ...
    useEffect(() => {
        ...
        // Check for accessid parameter and redirect to loading
        ...
        const initialPage = pathMap[path] || 'Home';
        setCurrentPage(initialPage);
    }, []);

    useEffect(() => {
        // Update URL when page changes (except loading)
        if (currentPage !== 'loading') {
            ...
            if (pageMap[currentPage] && window.location.pathname !== pageMap[currentPage]) {
                window.history.pushState({}, '', pageMap[currentPage]);
            }
        }
    }, [currentPage]);
